...
uses UModuloPrincipal, UAcesso, UModuloRelatorios;
...
procedure TFrmCadPadrao.ActAltRelExecute(Sender: TObject);
begin
...
procedure TFrmCadPadrao.ActImpExecute(Sender: TObject);
begin
  ModuloRelatorios.CarregaRelatorio(frxPadrao,_relpadrao, true);
end;
...
procedure TFrmCadPadrao.FormActivate(Sender: TObject);
begin
  pintaCampos;
end;
...
var i:integer;
begin
  if ModuloPrincipal._perfiUsuario > 1 then
    UAcesso.ConfigurarAcesso(Self, Self.Name, ModuloPrincipal.Conexao,
                             ModuloPrincipal._loginDoUsuario,ModuloPrincipal._codigoEmpresa);
  cdsPadrao.Close;
  cdsPadrao.Open;
...
begin
 ...
 {   if (Self.Components[i] is TLabel) then
    ...
    begin
      ...
      begin
          if Moduloprincipal.cdsParamssis_cor_cam_obr.asinteger > 0 then
            SetPropValue(self.Components[I], 'Color',Moduloprincipal.cdsParamssis_cor_cam_obr.asinteger)
          else
...
begin
  Moduloprincipal.ShowStatusBar(msg);
end;
